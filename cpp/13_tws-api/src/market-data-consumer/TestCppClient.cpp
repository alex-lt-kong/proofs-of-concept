...
void TestCppClient::tickDataOperation() {
    ...
    //! [reqmktdata]
    m_pClient->reqMktData(1001, ContractSamples::StockComboContract(), "", false,
                          false, TagValueListSPtr());
    m_pClient->reqMktData(1002, ContractSamples::OptionWithLocalSymbol(), "",
                          false, false, TagValueListSPtr());
    //! [reqmktdata]
    //! [reqmktdata_snapshot]
    m_pClient->reqMktData(1003, ContractSamples::FutureComboContract(), "", true,
                          false, TagValueListSPtr());
    //! [reqmktdata_snapshot]
    ...
    */
    ...
    // type" error
    m_pClient->reqMktData(1005, ContractSamples::USStock(), "mdoff,292:BZ", false,
                          false, TagValueListSPtr());
    m_pClient->reqMktData(1006, ContractSamples::USStock(), "mdoff,292:BT", false,
                          false, TagValueListSPtr());
    ...
    // flag
    m_pClient->reqMktData(1015, ContractSamples::SimpleFuture(), "", false, false,
                          TagValueListSPtr());
    //! [reqpreopenbidask]
    ...
    //! [IPOPrice]
    m_pClient->reqMktData(1019, ContractSamples::StockWithIPOPrice(), "mdoff,586",
                          false, false, TagValueListSPtr());
    //! [IPOPrice]
...
void TestCppClient::tickPrice(TickerId tickerId, TickType field, double price,
    ...
    else if (field == TickType::DELAYED_HIGH)
        tickTypeName = "DELAYED_HIGH";
    else if (field == TickType::DELAYED_LOW)
        tickTypeName = "DELAYED_LOW";
    spdlog::info("tickPrice(). Ticker Id: {}, TickType: {}, Price: {}, CanAutoExecute: {}, PastLimit: {}, PreOpen: {}",
                 tickerId, tickTypeName, Utils::doubleMaxString(price), attribs.canAutoExecute,
                 attribs.pastLimit,
                 attribs.preOpen);
}
...
void TestCppClient::tickSize(TickerId tickerId, TickType field, Decimal size) {
    ...
    spdlog::info("tickSize() Ticker Id: {}, TickType: {}, Size: {}", tickerId,
...
void TestCppClient::tickString(TickerId tickerId, TickType tickType,
    ...
    if (tickType == DELAYED_LAST_TIMESTAMP) {
        ...
        spdlog::info("tickString(DELAYED_LAST_TIMESTAMP) Ticker Id: {}, Value: {}", tickerId, oss.str());
        ...
        std::ostringstream oss;
        oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
        spdlog::info("tickString(LAST_TIMESTAMP) Ticker Id: {}, Value: {}", tickerId, oss.str());
...
void TestCppClient::historicalTicks(int reqId,
    ...
    for (const HistoricalTick &tick: ticks) {
        ...
        oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
        spdlog::info(
            "historicalTicks(). ReqId: {}, Time: {}, price: {}, "
            "size: {}", reqId, oss.str(), Utils::doubleMaxString(tick.price),
...
void TestCppClient::historicalTicksLast(
    ...
    for (HistoricalTickLast tick: ticks) {
        ...
        oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
        spdlog::info(
            "historicalTicksLast() ReqId: {}, Time: {}, Price: {}, "
            "Size: {}, Exchange: {}, "
            "SpecialConditions: {}, unreported: {}, pastLimit: {}",
            reqId, oss.str(),
            Utils::doubleMaxString(tick.price).c_str(),
            DecimalFunctions::decimalStringToDisplay(tick.size).c_str(),
            tick.exchange,
            tick.specialConditions, tick.tickAttribLast.unreported, tick.tickAttribLast.pastLimit);
    }
}
...
void TestCppClient::tickByTickAllLast(int reqId, int tickType, time_t time,
    ...
    spdlog::info(
        ...
        "SpecialConditions:{}",
        reqId, (tickType == 1 ? "Last" : "AllLast"), oss.str(), Utils::doubleMaxString(price),
        DecimalFunctions::decimalStringToDisplay(size), tickAttribLast.pastLimit, tickAttribLast.unreported, exchange,
        specialConditions);
}
