...
void TestCppClient::tickDataOperation() {
  ...
  */
  ...
  // type" error
  m_pClient->reqMktData(1005, ContractSamples::UsStkNvda(), "mdoff,292:BZ",
                        false, false, TagValueListSPtr());
  m_pClient->reqMktData(1006, ContractSamples::UsStkNvda(), "mdoff,292:BT",
                        false, false, TagValueListSPtr());
  m_pClient->reqMktData(1007, ContractSamples::UsStkNvda(), "mdoff,292:FLY",
                        false, false, TagValueListSPtr());
  m_pClient->reqMktData(1008, ContractSamples::UsStkNvda(), "mdoff,292:DJ-RT",
                        false, false, TagValueListSPtr());
  //! [reqmktdata_contractnews]
  ...
  m_pClient->reqMktData(1017, ContractSamples::UsEtfQqq(), "mdoff,577,614,623",
                        false, false, TagValueListSPtr());
  //! [reqetfticks]
...
void TestCppClient::tickPrice(TickerId tickerId, TickType field, double price,
  ...
  else if (field == TickType::DELAYED_OPEN)
    tickTypeName = "DELAYED_OPEN";
  else if (field == TickType::DELAYED_CLOSE)
    tickTypeName = "DELAYED_CLOSE";
  spdlog::info("tickPrice(). Symbol: {:18}, TickType: {:15}, Price: {:>7}",
               std::format("{:6} ({})", m_subscribedContracts[tickerId].symbol,
...
void TestCppClient::tickString(TickerId tickerId, TickType tickType,
  ...
  if (tickType == DELAYED_LAST_TIMESTAMP) {
    ...
    spdlog::info("tickString(DELAYED_LAST_TIMESTAMP) Ticker Id: {}, Value: {}",
                 tickerId, oss.str());
    ...
    std::ostringstream oss;
    oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
    spdlog::info("tickString(LAST_TIMESTAMP) Ticker Id: {}, Value: {}",
                 tickerId, oss.str());
...
void TestCppClient::historicalTicks(int reqId,
  ...
  for (const HistoricalTick &tick : ticks) {
    ...
    std::ostringstream oss;
    oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
    spdlog::info("historicalTicks(). ReqId: {}, Time: {}, price: {}, "
                 "size: {}",
...
void TestCppClient::historicalTicksLast(
  ...
  for (HistoricalTickLast tick : ticks) {
    ...
    std::ostringstream oss;
    oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
    spdlog::info("historicalTicksLast() ReqId: {}, Time: {}, Price: {}, "
                 "Size: {}, Exchange: {}, "
