...
bool TestCppClient::connect(const char *host, int port, int clientId) {
    ...
    if (bRes) {
        ...
        //! [ereader]
        m_pReader =
                std::unique_ptr<EReader>(new EReader(m_pClient, &m_osSignal));
        m_pReader->start();
        //! [ereader]
    } else
...
void TestCppClient::tickDataOperation() {
    std::cout << "tickDataOperation()" << std::endl;
    //! [reqmarketdatatype]
    ...
    //! [reqmktdata]
    m_pClient->reqMktData(1001, ContractSamples::StockComboContract(), "",
                          false, false, TagValueListSPtr());
    m_pClient->reqMktData(1002, ContractSamples::OptionWithLocalSymbol(), "",
                          false, false, TagValueListSPtr());
    //! [reqmktdata]
    //! [reqmktdata_snapshot]
    m_pClient->reqMktData(1003, ContractSamples::FutureComboContract(), "",
                          true, false, TagValueListSPtr());
    //! [reqmktdata_snapshot]

    /*
    //! [regulatorysnapshot]
    // Each regulatory snapshot incurs a fee of 0.01 USD
    m_pClient->reqMktData(1013, ContractSamples::USStock(), "", false, true,
    TagValueListSPtr());
    //! [regulatorysnapshot]
    */
    ...
    m_pClient->reqMktData(1004, ContractSamples::USStockAtSmart(), "233,236",
                          false, false, TagValueListSPtr());
    //! [reqmktdata_genticks]

    //! [reqmktdata_contractnews]
    // Without the API news subscription this will generate an "invalid tick
    // type" error
    m_pClient->reqMktData(1005, ContractSamples::USStock(), "mdoff,292:BZ",
                          false, false, TagValueListSPtr());
    m_pClient->reqMktData(1006, ContractSamples::USStock(), "mdoff,292:BT",
                          false, false, TagValueListSPtr());
    m_pClient->reqMktData(1007, ContractSamples::USStock(), "mdoff,292:FLY",
                          false, false, TagValueListSPtr());
    m_pClient->reqMktData(1008, ContractSamples::USStock(), "mdoff,292:DJ-RT",
                          false, false, TagValueListSPtr());
    //! [reqmktdata_contractnews]
    //! [reqmktdata_broadtapenews]
    m_pClient->reqMktData(1009, ContractSamples::BTbroadtapeNewsFeed(),
                          "mdoff,292", false, false, TagValueListSPtr());
    m_pClient->reqMktData(1010, ContractSamples::BZbroadtapeNewsFeed(),
                          "mdoff,292", false, false, TagValueListSPtr());
    m_pClient->reqMktData(1011, ContractSamples::FLYbroadtapeNewsFeed(),
                          "mdoff,292", false, false, TagValueListSPtr());
    //! [reqmktdata_broadtapenews]

    //! [reqoptiondatagenticks]
    // Requesting data for an option contract will return the greek values
    m_pClient->reqMktData(1013, ContractSamples::USOptionContract(), "", false,
                          false, TagValueListSPtr());
    //! [reqoptiondatagenticks]

    //! [reqfuturesopeninterest]
    // Requesting data for a futures contract will return the futures open
    // interest
    m_pClient->reqMktData(1014, ContractSamples::SimpleFuture(), "mdoff,588",
                          false, false, TagValueListSPtr());
    //! [reqfuturesopeninterest]

    //! [reqpreopenbidask]
    // Requesting data for a futures contract will return the pre-open bid/ask
    // flag
    m_pClient->reqMktData(1015, ContractSamples::SimpleFuture(), "", false,
                          false, TagValueListSPtr());
    //! [reqpreopenbidask]

    //! [reqavgoptvolume]
    // Requesting data for a stock will return the average option volume
    m_pClient->reqMktData(1016, ContractSamples::USStockAtSmart(), "mdoff,105",
                          false, false, TagValueListSPtr());
    //! [reqavgoptvolume]

    //! [reqetfticks]
    // Requesting data for an ETF will return the ETF ticks
    m_pClient->reqMktData(1017, ContractSamples::etf(), "mdoff,577,614,623",
                          false, false, TagValueListSPtr());
    //! [reqetfticks]

    //! [reqmktdatacrypto]
    m_pClient->reqMktData(1018, ContractSamples::CryptoContract(), "", false,
                          false, TagValueListSPtr());
    //! [reqmktdatacrypto]

    //! [IPOPrice]
    m_pClient->reqMktData(1019, ContractSamples::StockWithIPOPrice(),
                          "mdoff,586", false, false, TagValueListSPtr());
    //! [IPOPrice]

    //! [yieldbidask]
    m_pClient->reqMktData(1020, ContractSamples::Bond(), "", false, false,
                          TagValueListSPtr());
    //! [yieldbidask]
...
void TestCppClient::optionsOperations() {
    ...
    //! [exercise_options]
    //** Exercising options ***
    m_pClient->exerciseOptions(5003, ContractSamples::OptionWithTradingClass(),
                               1, 1, "", 1, "20231018-12:00:00", "CustAcct",
                               true);
    //! [exercise_options]
...
void TestCppClient::marketScanners() {
    /*** Requesting all available parameters which can be used to build a
     * scanner request ***/
    ...
    m_pClient->reqScannerSubscription(
            7001, ScannerSubscriptionSamples::HotUSStkByVolume(),
    ...
    m_pClient->reqScannerSubscription(
            7002, ScannerSubscriptionSamples::HotUSStkByVolume(),
            TagValueListSPtr(),
...
void TestCppClient::fundamentals() {
    ...
    //! [reqfundamentaldata]
    m_pClient->reqFundamentalData(8001, ContractSamples::USStock(),
...
void TestCppClient::orderOperations() {
     ...
     * ***/
    //! [order_submission]
    m_pClient->placeOrder(
            m_orderId++, ContractSamples::USStock(),
            OrderSamples::LimitOrder(
                    "SELL", DecimalFunctions::stringToDecimal("1"), 50));
    //! [order_submission]

    // m_pClient->placeOrder(m_orderId++, ContractSamples::OptionAtBox(),
    // OrderSamples::Block("BUY", DecimalFunctions::stringToDecimal("50"), 20));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::OptionAtBox(),
    // OrderSamples::BoxTop("SELL", DecimalFunctions::stringToDecimal("10")));
    // m_pClient->placeOrder(m_orderId++,
    // ContractSamples::FutureComboContract(),
    // OrderSamples::ComboLimitOrder("SELL",
    // DecimalFunctions::stringToDecimal("1"), 1, false));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::StockComboContract(),
    // OrderSamples::ComboMarketOrder("BUY",
    // DecimalFunctions::stringToDecimal("1"), false));
    // m_pClient->placeOrder(m_orderId++,
    // ContractSamples::OptionComboContract(),
    // OrderSamples::ComboMarketOrder("BUY",
    // DecimalFunctions::stringToDecimal("1"), true));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::StockComboContract(),
    // OrderSamples::LimitOrderForComboWithLegPrices("BUY",
    // DecimalFunctions::stringToDecimal("1"), std::vector<double>(10, 5),
    // true)); m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::Discretionary("SELL",
    // DecimalFunctions::stringToDecimal("1"), 45, 0.5));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::OptionAtBox(),
    // OrderSamples::LimitIfTouched("BUY",
    // DecimalFunctions::stringToDecimal("1"), 30, 34));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::LimitOnClose("SELL",
    // DecimalFunctions::stringToDecimal("1"), 34));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::LimitOnOpen("BUY", DecimalFunctions::stringToDecimal("1"),
    // 35)); m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::MarketIfTouched("BUY",
    // DecimalFunctions::stringToDecimal("1"), 35));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::MarketOnClose("SELL",
    // DecimalFunctions::stringToDecimal("1")));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::MarketOnOpen("BUY",
    // DecimalFunctions::stringToDecimal("1")));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::MarketOrder("SELL",
    // DecimalFunctions::stringToDecimal("1")));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::MarketToLimit("BUY",
    // DecimalFunctions::stringToDecimal("1")));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::OptionAtIse(),
    // OrderSamples::MidpointMatch("BUY",
    // DecimalFunctions::stringToDecimal("1")));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::Stop("SELL",
    // DecimalFunctions::stringToDecimal("1"), 34.4));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::StopLimit("BUY", DecimalFunctions::stringToDecimal("1"),
    // 35, 33)); m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::StopWithProtection("SELL",
    // DecimalFunctions::stringToDecimal("1"), 45));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::SweepToFill("BUY", DecimalFunctions::stringToDecimal("1"),
    // 35)); m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::TrailingStop("SELL",
    // DecimalFunctions::stringToDecimal("1"), 0.5, 30));
    // m_pClient->placeOrder(m_orderId++, ContractSamples::USStock(),
    // OrderSamples::TrailingStopLimit("BUY",
    // DecimalFunctions::stringToDecimal("100"), 2, 5, 50));

    //! [place_midprice]
    m_pClient->placeOrder(
            m_orderId++, ContractSamples::USStockAtSmart(),
            OrderSamples::Midprice(
                    "BUY", DecimalFunctions::stringToDecimal("1"), 150));
    //! [place_midprice]

    //! [place order with cashQty]
    m_pClient->placeOrder(
            m_orderId++, ContractSamples::USStockAtSmart(),
            OrderSamples::LimitOrderWithCashQty("BUY", 111.11, 5000));
    //! [place order with cashQty]
    ...
    //! [order_submission]
    m_pClient->placeOrder(
            m_orderId++, ContractSamples::CryptoContract(),
            OrderSamples::LimitOrder(
                    "BUY", DecimalFunctions::stringToDecimal("0.12345678"),
                    3700));
    //! [order_submission]

    //! [manual_order_time]
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(),
                          OrderSamples::LimitOrderWithManualOrderTime(
                                  "BUY",
                                  DecimalFunctions::stringToDecimal("100"),
                                  111.11, "20220314-13:00:00"));
    //! [manual_order_time]

    //! [manual_order_cancel_time]
    m_pClient->cancelOrder(
            m_orderId - 1,
            OrderSamples::OrderCancelWithManualTime("20240614-00:00:05"));
    //! [manual_order_cancel_time]

    //! [pegbest_up_to_mid_order_submission]
    m_pClient->placeOrder(m_orderId++, ContractSamples::IBKRATSContract(),
                          OrderSamples::PegBestUpToMidOrder(
                                  "BUY",
                                  DecimalFunctions::stringToDecimal("100"),
                                  111.11, 100, 200, 0.02, 0.025));
    //! [pegbest_up_to_mid_order_submission]

    //! [pegbest_order_submission]
    m_pClient->placeOrder(
            m_orderId++, ContractSamples::IBKRATSContract(),
            OrderSamples::PegBestOrder("BUY",
                                       DecimalFunctions::stringToDecimal("100"),
                                       111.11, 100, 200, 0.03));
    //! [pegbest_order_submission]
    ...
    m_pClient->placeOrder(
            m_orderId++, ContractSamples::IBKRATSContract(),
            OrderSamples::PegMidOrder("BUY",
                                      DecimalFunctions::stringToDecimal("100"),
                                      111.11, 100, 0.02, 0.025));
    //! [pegmid_order_submission]

    //! [customer_account]
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(),
                          OrderSamples::LimitOrderWithCustomerAccount(
                                  "BUY",
                                  DecimalFunctions::stringToDecimal("100"),
                                  111.11, "CustAcct"));
    //! [customer_account]
...
void TestCppClient::ocaSamples() {
    ...
    ocaOrders.push_back(OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("1"), 10));
    ocaOrders.push_back(OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("1"), 11));
    ocaOrders.push_back(OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("1"), 12));
...
void TestCppClient::conditionSamples() {
    ...
    Order lmt = OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("100"), 10);
    // Order will become active if conditioning criteria is met
    PriceCondition *priceCondition =
            dynamic_cast<PriceCondition *>(OrderSamples::Price_Condition(
                    208813720, "SMART", 600, false, false));
    ExecutionCondition *execCondition = dynamic_cast<ExecutionCondition *>(
            OrderSamples::Execution_Condition("EUR.USD", "CASH", "IDEALPRO",
                                              true));
    MarginCondition *marginCondition = dynamic_cast<MarginCondition *>(
            OrderSamples::Margin_Condition(30, true, false));
    PercentChangeCondition *pctChangeCondition =
            dynamic_cast<PercentChangeCondition *>(
                    OrderSamples::Percent_Change_Condition(
                            15.0, 208813720, "SMART", true, true));
    TimeCondition *timeCondition =
            dynamic_cast<TimeCondition *>(OrderSamples::Time_Condition(
                    "20220808 10:00:00 US/Eastern", true, false));
    VolumeCondition *volumeCondition =
            dynamic_cast<VolumeCondition *>(OrderSamples::Volume_Condition(
                    208813720, "SMART", false, 100, true));

    lmt.conditions.push_back(std::shared_ptr<PriceCondition>(priceCondition));
    lmt.conditions.push_back(
            std::shared_ptr<ExecutionCondition>(execCondition));
    lmt.conditions.push_back(std::shared_ptr<MarginCondition>(marginCondition));
    lmt.conditions.push_back(
            std::shared_ptr<PercentChangeCondition>(pctChangeCondition));
    ...
    Order lmt2 = OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("100"), 20);
    // The active order will be cancelled if conditioning criteria is met
    lmt2.conditionsCancelOrder = true;
    PriceCondition *priceCondition2 =
            dynamic_cast<PriceCondition *>(OrderSamples::Price_Condition(
                    208813720, "SMART", 600, false, false));
...
void TestCppClient::bracketSample() {
    ...
    Order stopLoss;
    //! [bracketsubmit]
    OrderSamples::BracketOrder(m_orderId++, parent, takeProfit, stopLoss, "BUY",
                               DecimalFunctions::stringToDecimal("100"), 30, 40,
                               20);
    m_pClient->placeOrder(parent.orderId, ContractSamples::EuropeanStock(),
                          parent);
    m_pClient->placeOrder(takeProfit.orderId, ContractSamples::EuropeanStock(),
                          takeProfit);
    m_pClient->placeOrder(stopLoss.orderId, ContractSamples::EuropeanStock(),
                          stopLoss);
    //! [bracketsubmit]
...
void TestCppClient::hedgeSample() {
    ...
    Order parent = OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("100"), 10);
...
void TestCppClient::testAlgoSamples() {
    //! [algo_base_order]
    Order baseOrder = OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("1000"), 1);
    //! [algo_base_order]

    //! [arrivalpx]
    AvailableAlgoParams::FillArrivalPriceParams(
            baseOrder, 0.1, "Aggressive", "09:00:00 US/Eastern",
    ...
    //! [darkice]
    AvailableAlgoParams::FillDarkIceParams(baseOrder, 10, "09:00:00 US/Eastern",
                                           "16:00:00 US/Eastern", true);
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(),
                          baseOrder);
    //! [darkice]
    ...
    // always defaulted to GMT
    AvailableAlgoParams::FillAccumulateDistributeParams(baseOrder, 10, 60, true,
                                                        true, 1, true, true,
                                                        "12:00:00", "16:00:00");
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(),
                          baseOrder);
    ...
    AvailableAlgoParams::FillClosePriceParams(baseOrder, 0.5, "Neutral",
                                              "12:00:00 US/Eastern", true);
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(),
                          baseOrder);
    //! [closepx]

    //! [pctvol]
    AvailableAlgoParams::FillPctVolParams(baseOrder, 0.5, "12:00:00 US/Eastern",
    ...
    AvailableAlgoParams::FillPriceVariantPctVolParams(
            baseOrder, 0.1, 0.05, 0.01, 0.2, "12:00:00 US/Eastern",
            "14:00:00 US/Eastern", true);
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(),
                          baseOrder);
    //! [pctvolpx]
    ...
    AvailableAlgoParams::FillSizeVariantPctVolParams(
            baseOrder, 0.2, 0.4, "12:00:00 US/Eastern", "14:00:00 US/Eastern",
            true);
    m_pClient->placeOrder(m_orderId++, ContractSamples::USStockAtSmart(),
                          baseOrder);
    //! [pctvolsz]
    ...
    AvailableAlgoParams::FillJefferiesVWAPParams(
            baseOrder, "10:00:00 US/Eastern", "16:00:00 US/Eastern", 10, 10,
            "Exclude_Both", 130, 135, 1, 10, "Patience", false, "Midpoint");
    m_pClient->placeOrder(m_orderId++, ContractSamples::JefferiesContract(),
                          baseOrder);
    //! [jeff_vwap_algo]

    //! [csfb_inline_algo]
    AvailableAlgoParams::FillCSFBInlineParams(
            baseOrder, "10:00:00 US/Eastern", "16:00:00 US/Eastern", "Patient",
            10, 20, 100, "Default", false, 40, 100, 100, 35);
    m_pClient->placeOrder(m_orderId++, ContractSamples::CSFBContract(),
                          baseOrder);
    //! [csfb_inline_algo]
...
void TestCppClient::financialAdvisorOrderSamples() {
    //! [faorderoneaccount]
    Order faOrderOneAccount = OrderSamples::MarketOrder(
            "BUY", DecimalFunctions::stringToDecimal("100"));
    ...
    //! [faordergroup]
    Order faOrderGroup = OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("200"), 10);
    ...
    Order faOrderUserDefinedGroup = OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("200"), 10);
    ...
    //! [modelorder]
    Order modelOrder = OrderSamples::LimitOrder(
            "BUY", DecimalFunctions::stringToDecimal("200"), 100);
...
void TestCppClient::financialAdvisorOperations() {
    ...
    m_pClient->replaceFA(1000, faDataType::GROUPS,
                         FAMethodSamples::FAUpdatedGroup());
    //! [replacefaupdatedgroup]
...
void TestCppClient::reqSmartComponents() {
    ...
    if (bFirstRun) {
        m_pClient->reqMktData(13001, ContractSamples::USStockAtSmart(), "",
                              false, false, TagValueListSPtr());
...
void TestCppClient::reqHeadTimestamp() {
    //! [reqHeadTimeStamp]
    m_pClient->reqHeadTimestamp(14001, ContractSamples::EurGbpFx(), "MIDPOINT",
                                1, 1);
    //! [reqHeadTimeStamp]
    std::this_thread::sleep_for(std::chrono::seconds(1));
    //! [cancelHeadTimestamp]
...
void TestCppClient::rerouteCFDOperations() {
    //! [reqmktdatacfd]
    m_pClient->reqMktData(16001, ContractSamples::USStockCFD(), "", false,
                          false, TagValueListSPtr());
    std::this_thread::sleep_for(std::chrono::seconds(1));
    ...
    m_pClient->reqMktDepth(16004, ContractSamples::USStockCFD(), 10, false,
                           TagValueListSPtr());
    std::this_thread::sleep_for(std::chrono::seconds(1));
    m_pClient->reqMktDepth(16005, ContractSamples::EuropeanStockCFD(), 10,
                           false, TagValueListSPtr());
    std::this_thread::sleep_for(std::chrono::seconds(1));
...
void TestCppClient::reqTickByTickData() {
    ...
    m_pClient->reqTickByTickData(20001, ContractSamples::EuropeanStock(),
                                 "Last", 0, false);
    ...
    //! [reqtickbytick]
    m_pClient->reqTickByTickData(20005, ContractSamples::EuropeanStock(),
                                 "Last", 10, false);
...
void TestCppClient::whatIfSamples() {
    ...
    //! [whatiforder]
    m_pClient->placeOrder(
            m_orderId++, ContractSamples::BondWithCusip(),
            OrderSamples::WhatIfLimitOrder(
                    "BUY", DecimalFunctions::stringToDecimal("100"), 20));
    //! [whatiforder]
...
void TestCppClient::ibkratsSample() {
    //! [ibkratssubmit]
    Order ibkratsOrder = OrderSamples::LimitIBKRATS(
            "BUY", DecimalFunctions::stringToDecimal("100"), 330);
    m_pClient->placeOrder(m_orderId++, ContractSamples::IBKRATSContract(),
                          ibkratsOrder);
    //! [ibkratssubmit]
...
void TestCppClient::tickOptionComputation(TickerId tickerId, TickType tickType,
    ...
    printf("TickOptionComputation. Ticker Id: %ld, Type: %d, TickAttrib: %s, "
           ...
           tickerId, (int) tickType, Utils::intMaxString(tickAttrib).c_str(),
           Utils::doubleMaxString(impliedVol).c_str(),
           Utils::doubleMaxString(delta).c_str(),
           Utils::doubleMaxString(optPrice).c_str(),
           Utils::doubleMaxString(pvDividend).c_str(),
           Utils::doubleMaxString(gamma).c_str(),
           Utils::doubleMaxString(vega).c_str(),
           Utils::doubleMaxString(theta).c_str(),
           Utils::doubleMaxString(undPrice).c_str());
}
//! [tickoptioncomputation]
...
void TestCppClient::tickEFP(TickerId tickerId, TickType tickType,
    ...
    printf("TickEFP. %ld, Type: %d, BasisPoints: %s, FormattedBasisPoints: %s, "
           ...
           Utils::doubleMaxString(basisPoints).c_str(),
...
void TestCppClient::openOrder(OrderId orderId, const Contract &contract,
    ...
    printf("OpenOrder. PermId: %s, ClientId: %s, OrderId: %s, Account: %s, "
           "Symbol: "
           "%s, SecType: %s, Exchange: %s:, Action: %s, OrderType:%s, "
           "TotalQty: %s, "
           "CashQty: %s, "
           "LmtPrice: %s, AuxPrice: %s, Status: %s, MinTradeQty: %s, "
           "MinCompeteSize: %s, CompeteAgainstBestOffset: %s, "
           "MidOffsetAtWhole: %s, "
           "MidOffsetAtHalf: %s, "
           "FAGroup: %s, FAMethod: %s, CustomerAccount: %s, "
           "ProfessionalCustomer: "
           "%s, BondAccruedInterest: %s\n",
           Utils::intMaxString(order.permId).c_str(),
           Utils::longMaxString(order.clientId).c_str(),
           Utils::longMaxString(orderId).c_str(), order.account.c_str(),
           contract.symbol.c_str(), contract.secType.c_str(),
           contract.exchange.c_str(), order.action.c_str(),
           order.orderType.c_str(),
           DecimalFunctions::decimalStringToDisplay(order.totalQuantity)
                   .c_str(),
           Utils::doubleMaxString(order.cashQty).c_str(),
           Utils::doubleMaxString(order.lmtPrice).c_str(),
           Utils::doubleMaxString(order.auxPrice).c_str(),
           orderState.status.c_str(),
           Utils::intMaxString(order.minTradeQty).c_str(),
           Utils::intMaxString(order.minCompeteSize).c_str(),
           order.competeAgainstBestOffset ==
                           COMPETE_AGAINST_BEST_OFFSET_UP_TO_MID
                   ? "UpToMid"
                   : Utils::doubleMaxString(order.competeAgainstBestOffset)
                             .c_str(),
           Utils::doubleMaxString(order.midOffsetAtWhole).c_str(),
           Utils::doubleMaxString(order.midOffsetAtHalf).c_str(),
           order.faGroup.c_str(), order.faMethod.c_str(),
           order.customerAccount.c_str(),
           (order.professionalCustomer ? "true" : "false"),
           order.bondAccruedInterest.c_str());
}
//! [openorder]
...
void TestCppClient::printContractDetailsMsg(
    ...
    if (contractDetails.contract.secType == "FUND") {
        ...
        printf("\t\tFundAssetType: %s\n",
               Utils::getFundAssetTypeName(contractDetails.fundAssetType)
                       .c_str());
    }
...
void TestCppClient::execDetails(int reqId, const Contract &contract,
    ...
    printf("ExecDetails. ReqId: %d - %s, %s, %s - %s, %s, %s, %s, %s, %s\n",
           ...
           Utils::longMaxString(execution.orderId).c_str(),
           DecimalFunctions::decimalStringToDisplay(execution.shares).c_str(),
           DecimalFunctions::decimalStringToDisplay(execution.cumQty).c_str(),
           Utils::intMaxString(execution.lastLiquidity).c_str(),
           (execution.pendingPriceRevision ? "yes" : "no"));
}
//! [execdetails]
...
void TestCppClient::deltaNeutralValidation(
    ...
    printf("DeltaNeutralValidation. %d, ConId: %ld, Delta: %s, Price: %s\n",
           ...
           Utils::doubleMaxString(deltaNeutralContract.delta).c_str(),
           Utils::doubleMaxString(deltaNeutralContract.price).c_str());
}
...
void TestCppClient::commissionReport(const CommissionReport &commissionReport) {
    printf("CommissionReport. %s - %s %s RPNL %s\n",
           ...
           Utils::doubleMaxString(commissionReport.realizedPNL).c_str());
}
//! [commissionreport]
...
void TestCppClient::accountUpdateMulti(int reqId, const std::string &account,
    ...
    printf("AccountUpdate Multi. Request: %d, Account: %s, ModelCode: %s, Key, "
           ...
           reqId, account.c_str(), modelCode.c_str(), key.c_str(),
           value.c_str(), currency.c_str());
}
//! [accountupdatemulti]
...
void TestCppClient::symbolSamples(
    ...
    for (unsigned int i = 0; i < contractDescriptions.size(); i++) {
        ...
        printf("Contract (%u): conId: %ld, symbol: %s, secType: %s, "
               ...
               i, contract.conId, contract.symbol.c_str(),
               contract.secType.c_str(), contract.primaryExchange.c_str(),
               contract.currency.c_str());
...
void TestCppClient::mktDepthExchanges(
    ...
    for (unsigned int i = 0; i < depthMktDataDescriptions.size(); i++) {
        printf("Depth Mkt Data Description [%d] - exchange: %s secType: %s "
               ...
               Utils::intMaxString(depthMktDataDescriptions[i].aggGroup)
                       .c_str());
    }
}
//! [mktDepthExchanges]
...
void TestCppClient::smartComponents(int reqId,
    ...
    for (SmartComponentsMap::const_iterator i = theMap.begin();
         i != theMap.end(); i++) {
...
void TestCppClient::tickReqParams(int tickerId, double minTick,
    ...
    printf("tickerId: %d, minTick: %s, bboExchange: %s, snapshotPermissions: "
           "%u\n",
           tickerId, Utils::doubleMaxString(minTick).c_str(),
...
void TestCppClient::pnl(int reqId, double dailyPnL, double unrealizedPnL,
                        double realizedPnL) {
    printf("PnL. ReqId: %d, daily PnL: %s, unrealized PnL: %s, realized PnL: "
           "%s\n",
...
void TestCppClient::historicalTicks(int reqId,
    ...
    for (const HistoricalTick &tick: ticks) {
        ...
        ctime_r(&t, timeStr);
#endif
        std::cout << "Historical tick. ReqId: " << reqId
                  << ", time: " << timeStr
                  << ", price: " << Utils::doubleMaxString(tick.price).c_str()
                  << ", size: "
...
void TestCppClient::historicalTicksBidAsk(
    ...
    for (const HistoricalTickBidAsk &tick: ticks) {
        ...
        spdlog::info("historicalTicksBidAsk(). ReqId: {}, Time: {}, bid: {}, "
                     "ask: {}, bidSize: {}, askSize: {}, bidPastLow: {}, "
                     "askPastHigh: {}",
                     reqId, oss.str(),
                     Utils::doubleMaxString(tick.priceBid).c_str(),
...
void TestCppClient::historicalTicksLast(
    ...
    for (HistoricalTickLast tick: ticks) {
        ...
        std::cout << "Historical tick last. ReqId: " << reqId
                  ...
                  << ", pastLimit: " << tick.tickAttribLast.pastLimit
                  << std::endl;
    }
}
//! [historicaltickslast]
...
void TestCppClient::tickByTickBidAsk(int reqId, time_t time, double bidPrice,
    ...
    std::ostringstream oss;
    oss << std::put_time(&tm, "%Y-%m-%dT%H:%M:%S%z");
    spdlog::info("tickByTickBidAsk(): ReqId: {}, TickType: BidAsk, Time: {}, "
                 "BidPrice: {}, "
...
void TestCppClient::orderBound(long long orderId, int apiClientId,
    ...
    printf("Order bound. OrderId: %s, ApiClientId: %s, ApiOrderId: %s\n",
           ...
           Utils::intMaxString(apiOrderId).c_str());
}
//! [orderbound]
...
void TestCppClient::completedOrder(const Contract &contract, const Order &order,
                                   const OrderState &orderState) {
    printf("CompletedOrder. PermId: %s, ParentPermId: %s, Account: %s, Symbol: "
           "%s, "
           "SecType: %s, Exchange: %s:, Action: %s, OrderType: %s, TotalQty: "
           "%s, "
           "CashQty: %s, FilledQty: %s, "
           "LmtPrice: %s, AuxPrice: %s, Status: %s, CompletedTime: %s, "
           "CompletedStatus: %s, MinTradeQty: %s, MinCompeteSize: %s, "
           "CompeteAgainstBestOffset: %s, MidOffsetAtWhole: %s, "
           "MidOffsetAtHalf: "
           "%s, "
           "CustomerAccount: %s, ProfessionalCustomer: %s\n",
           Utils::intMaxString(order.permId).c_str(),
           Utils::llongMaxString(order.parentPermId).c_str(),
           order.account.c_str(), contract.symbol.c_str(),
           contract.secType.c_str(), contract.exchange.c_str(),
           order.action.c_str(), order.orderType.c_str(),
           DecimalFunctions::decimalStringToDisplay(order.totalQuantity)
                   .c_str(),
           Utils::doubleMaxString(order.cashQty).c_str(),
           DecimalFunctions::decimalStringToDisplay(order.filledQuantity)
                   .c_str(),
           Utils::doubleMaxString(order.lmtPrice).c_str(),
           Utils::doubleMaxString(order.auxPrice).c_str(),
...
//! [completedordersend]
void TestCppClient::completedOrdersEnd() { printf("CompletedOrdersEnd\n"); }
//! [completedordersend]
