...
int main(int argc, char** argv)
{
	...
	for (;;) {
	  ...
	  /*
		...
		// Request market data
		// Parameters: tickerId (unique identifier), contract, genericTickList, snapshot, regulatorySnapshot
		client.m_pClient->reqMktData(1, ContractSamples::EurGbpFx(), "", false, false, TagValueListSPtr());
	        //client.m_pClient->reqTickByTickData(1, contract, "Last", 10, false);
		//client.m_pClient->allowRedirect()
		while( client.isConnected()) {
			//client.tickDataOperation();
			client.processMessages();
			std::cout << "client.processMessages()\n";
		}
